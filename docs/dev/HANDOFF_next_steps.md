...
État au moment de l'écriture : branche `development`, dernier commit poussé `a017571` (vérifier avec `git log --oneline -5`). Suite de tests : 2060 exemples, 97,71 % de couverture (après avoirs, édition de l'entité, PDF, e-mail, amortissements, totaux du formulaire de facture, immobilisations depuis une facture, aides contextuelles, duplication de facture et dates fiables), un seul échec connu et flaky (voir "Pièges").
...
- **#11** Formulaire de facture plus léger : **dupliquer et dates fiables — FAIT (2026-09-25)** ; factures récurrentes à faire ; les « modèles » sont couverts par la duplication (on duplique la dernière facture du client).
  - **Dupliquer** : bouton « Duplicate » sur une facture (pas un avoir), `Accounting::DuplicateInvoice` (tout ou rien) : brouillon à la date du jour dans l'exercice ouvert (`FiscalYear.current`, refus sans exercice ouvert), même intervalle échéance/facture que l'original, copie du partenaire, type, journal, devise et taux, régime TVA, description, notes, lignes et **annotations analytiques**. Non copiés : numéro, écriture, Peppol, `external_ref`, journal de caisse, facture créditée. S'ouvre en édition. Réservé à ceux qui peuvent créer une facture (`InvoicePolicy#duplicate?` = `create?`).
  - **Dates** : `Partner#payment_terms_days` (entier ≥ 0, défaut 30, champ dans la fiche partenaire). Nouvelle facture : date du jour préremplie (`InvoicesController#new`) ; l'échéance = date + délai du partenaire, mise à jour au changement de partenaire ou de date tant qu'elle n'est pas saisie à la main (`invoice_form_controller.js` : `fillDueDate`, `markDueDateTouched`, `partnerTermsDays`, options du sélecteur avec `data-days` ; une échéance déjà présente au chargement n'est jamais écrasée). Côté serveur, `Invoice#default_due_date` (à la création, factures seulement, pas les avoirs) donne date + délai quand aucune échéance n'est saisie : indépendant du JavaScript.
  - Testé : `duplicate_invoice_spec` (quatre mutations détectées), requêtes, et `spec/system/accounting/invoice_form_dates_spec.rb` (Chrome headless).
  - **Reste : factures récurrentes** (il faut un modèle de récurrence, une tâche planifiée Solid Queue : `config/recurring.yml` n'existe pas, et un circuit de validation, le brouillon généré ne doit jamais être posté seul). Hors périmètre : délai de paiement par défaut au niveau de l'entité, conditions de paiement multiples (acompte, escompte). Piège de test : `let` paresseux créé dans un bloc `expect { ... }.to change(...)` compte dans le changement, créer l'objet avant.
