...
# from their source document, and lettered lines must be unlettered first.
class Accounting::ReverseJournalEntry
  def self.call(entry:)
    ctx = LightService::Context.make(entry: entry, reversal: nil)
    refusal = refusal_for(entry)
  ...
  def self.refusal_for(entry)
    ...
    return I18n.t("accounting.errors.reverse_has_source")  if entry.source_type.present?
